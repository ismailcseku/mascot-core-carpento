<?php
namespace MASCOTCORECARPENTO\CPT\Footer;
 ...
 * Singleton class
 * class CPT_Footer
 * @package MASCOTCORECARPENTO\CPT\Footer;
 */
final class CPT_Footer implements Lib\Mascot_Core_Carpento_Interface_PostType {
	...
	private function __construct() {
		$this->ptSingularName = esc_html__( 'Parts - Footer', 'mascot-core-carpento' );
		$this->ptPluralName = esc_html__( 'Parts - Footer', 'mascot-core-carpento' );
		$this->ptKey = 'footer';
	...
	public function customColumnsSettings( $columns ) {
		...
		$columns = array(
			...
			'active-footer'	=> esc_html__( 'Currently Active Footer', 'mascot-core-carpento' ),
		...
		switch( $columns ) {
			case 'active-footer' :
				if( mascot_core_carpento_theme_installed() ) {
					$active_footer_id = carpento_get_redux_option( 'footer-settings-choose-footer-widget-area', 'default' );

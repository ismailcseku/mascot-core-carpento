<?php
namespace MASCOTCORECARPENTO\CPT\SidePushPanel;
 ...
 * @package MASCOTCORECARPENTO\CPT\SidePushPanel;
 */
final class CPT_SidePushPanel implements Lib\Mascot_Core_Carpento_Interface_PostType {
	...
	private function __construct() {
		$this->ptSingularName = esc_html__( 'Parts - Side Push Panel', 'mascot-core-carpento' );
		$this->ptPluralName = esc_html__( 'Parts - Side Push Panel', 'mascot-core-carpento' );
	...
	public function customColumnsSettings( $columns ) {
		...
		$columns = array(
			...
			'active-side-push-panel'	=> esc_html__( 'Currently Active Side Push Panel', 'mascot-core-carpento' ),
		...
		switch( $columns ) {
			case 'active-side-push-panel' :
				if( mascot_core_carpento_theme_installed() ) {
					$active_headertop_id = carpento_get_redux_option( 'header-settings-choose-side-push-panel-cpt-widget-area', 'default' );

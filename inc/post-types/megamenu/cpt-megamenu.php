<?php
namespace MASCOTCORECARPENTO\CPT\MegaMenu;
 ...
 * Singleton class
 * class CPT_MegaMenu
 * @package MASCOTCORECARPENTO\CPT\MegaMenu;
 */
final class CPT_MegaMenu implements Lib\Mascot_Core_Carpento_Interface_PostType {
	...
	private function __construct() {
		$this->ptSingularName = esc_html__( 'Parts - Mega Menu', 'mascot-core-carpento' );
		$this->ptPluralName = esc_html__( 'Parts - Mega Menu', 'mascot-core-carpento' );
		$this->ptKey = 'megamenu';
	...
	public function customColumnsSettings( $columns ) {
		...
		$columns = array(
			...
			'active-megamenu'	=> esc_html__( 'Currently Active Mega Menu', 'mascot-core-carpento' ),
		...
		switch( $columns ) {
			case 'active-megamenu' :
				if( mascot_core_carpento_theme_installed() ) {
					$active_headertop_id = carpento_get_redux_option( 'megamenu-settings-choose-megamenu-cpt-widget-area', 'default' );
